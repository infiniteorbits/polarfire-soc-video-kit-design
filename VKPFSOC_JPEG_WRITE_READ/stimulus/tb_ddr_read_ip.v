...
--   - Checker temps reel    : compare data_o vs exp_data (8 bits)
--   - Timeout de securite
******************************************************************************/
`timescale 1ns/1ns
...
// Parametres globaux
// =========================================================================
parameter HORI_RES           = 1920/4  ; // 480 pixels par ligne
parameter LINES              = 4       ; // lignes par frame
parameter FRAMES             = 2       ; // nombre de frames a tester
...
// Variables checker
// =========================================================================
reg  [OUTPUT_DATA_WIDTH-1:0] exp_data  ;
reg                          error     ;
integer                      pixel_cnt ;
integer                      frame_cnt ;
...
initial begin
    ...
    $display("########################################################");
    if (error == 1'b0) begin
        $display("[TB] ---------- TEST PASSE avec succes ----------");
        $display("[TB] %0d pixels verifies, tous corrects", pixel_cnt);
    end else begin
        $display("[TB] ---------- TEST ECHOUE --------------------");
        $display("[TB] Voir messages [ERREUR] ci-dessus pour details");
    end
    $display("########################################################");
    #100 $stop;
end
...
// =========================================================================
// Checker temps reel (domaine pixel_clk_i)
// exp_data incremente de 1 a chaque pixel valide, wrap 255->1
// =========================================================================
initial exp_data = 8'h01;  // premier pixel attendu = 1

always @(posedge pixel_clk_i) begin
    if (data_valid_o) begin

        if (data_o !== exp_data) begin
            $display("[ERREUR] pixel=%0d | recu=0x%02h | attendu=0x%02h",
                      pixel_cnt, data_o, exp_data);
            error <= 1'b1;
        end
        ...
        if (exp_data == 8'hFF)
            ...
            exp_data <= exp_data + 8'h01;

        pixel_cnt <= pixel_cnt + 1;

        // Progression affichee toutes les HORI_RES pixels (= 1 ligne)
        if ((pixel_cnt + 1) % HORI_RES == 0)
            $display("[CHECK] Ligne %0d OK | %0d pixels verifies",
                      (pixel_cnt + 1) / HORI_RES, pixel_cnt + 1);
    end
end
