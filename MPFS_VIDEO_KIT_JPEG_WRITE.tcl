...
set constraint_path ./script_support/constraints/JPEG_WRITE
set project_name "VKPFSOC_JPEG_WRITE"
set project_dir "./$project_name"
source ./script_support/additional_configurations/functions_JPEG_WRITE.tcl
...
file copy $src_path/MSS_VIDEO_KIT/JPEG_WRITE/ $project_dir/MSS_VIDEO_KIT/


#
# // Generate base design
#
exec $mss_config_loc -GENERATE -CONFIGURATION_FILE:$src_path/MSS_VIDEO_KIT/JPEG_WRITE/MSS_VIDEO_KIT_JPEG_WRITE.cfg -OUTPUT_DIR:${src_path}/MSS_VIDEO_KIT/JPEG_WRITE/
import_mss_component -file "$src_path/MSS_VIDEO_KIT/JPEG_WRITE/MSS_VIDEO_KIT_H264.cxz"


#This Tcl file sources other Tcl files to build the design(on which recursive export is run) in a bottom-up fashion

#Sourcing the Tcl file in which all the HDL source files used in the design are imported or linked
source ${src_path}/hdl_source_JPEG_WRITE.tcl
build_design_hierarchy

#Sourcing the Tcl files in which HDL+ core definitions are created for HDL modules
source ${src_path}/components/JPEG_WRITE/video_fifo.tcl
source ${src_path}/components/JPEG_WRITE/data_packer.tcl
build_design_hierarchy

#Sourcing the Tcl files for creating individual ${src_path}/components under the top level
source ${src_path}/components/JPEG_WRITE/CORERESET.tcl 
source ${src_path}/components/JPEG_WRITE/CORERESET_PF_C5.tcl 
source ${src_path}/components/JPEG_WRITE/INIT_MONITOR.tcl 
source ${src_path}/components/JPEG_WRITE/PF_CCC_C0.tcl 
source ${src_path}/components/JPEG_WRITE/PF_CLK_DIV_C0.tcl 
source ${src_path}/components/JPEG_WRITE/PF_OSC_C0.tcl 
source ${src_path}/components/JPEG_WRITE/PF_XCVR_REF_CLK_C0.tcl 
source ${src_path}/components/JPEG_WRITE/CLOCKS_AND_RESETS.tcl 
source ${src_path}/components/JPEG_WRITE/CoreAPB3_C0.tcl 
source ${src_path}/components/JPEG_WRITE/FIC_CONVERTER.tcl 
source ${src_path}/components/JPEG_WRITE/DDR_AXI4_ARBITER_PF_C0.tcl 
source ${src_path}/components/JPEG_WRITE/DDR_WRITE_JPEG.tcl 
source ${src_path}/components/JPEG_WRITE/jpeg_top.tcl 
source ${src_path}/components/JPEG_WRITE/top_ddr_write.tcl 
source ${src_path}/components/JPEG_WRITE/VKPFSOC_TOP.tcl 
build_design_hierarchy
...
import_files \
	...
	-io_pdc "${constraint_path}/io/test_ddr_write.pdc" \
...
organize_tool_files -tool {PLACEROUTE} \
    ...
    -file "${project_dir}/constraint/io/test_ddr_write.pdc" \
    -file "${project_dir}/constraint/VKPFSOC_TOP_derived_constraints.sdc" \
...
if {[info exists HSS_UPDATE]} {
  if !{[file exists "./script_support/MSS_VIDEO_KIT/JPEG_WRITE/hss-envm-wrapper.mpfs-video-kit.hex"]} {
      if {[catch    {exec wget https://github.com/polarfire-soc/hart-software-services/releases/latest/download/hss-envm-wrapper.mpfs-video-kit.hex -P ./script_support/MSS_VIDEO_KIT/JPEG_WRITE} issue]} {
      }
     }
  create_eNVM_config "$local_dir/script_support/MSS_VIDEO_KIT/JPEG_WRITE/ENVM.cfg" "$local_dir/script_support/MSS_VIDEO_KIT/JPEG_WRITE/hss-envm-wrapper.mpfs-video-kit.hex"
  run_tool -name {GENERATEPROGRAMMINGDATA}
  configure_envm -cfg_file {script_support/MSS_VIDEO_KIT/JPEG_WRITE/ENVM.cfg}
}

if {[info exists SPIFLASH_DATA]} {
    if {$my_platform == "Linux"} {
	if {[catch {exec python generate_overlays_spiclient_data.py JPEG_WRITE} issue]} {}
    } else {
	file copy $local_dir/script_support/dtso_overlay/JPEG_WRITE/mpfs_JPEG_WRITE.dtbo  $local_dir/mpfs_dtbo.bin
	if {[catch {exec cmd.exe /c "wsl.exe -e python generate_overlays_spiclient_data.py JPEG_WRITE"} issue]} {}
    }
    create_spiflash "$project_dir/MSS_VIDEO_KIT/JPEG_WRITE/spiflash.cfg" $local_dir    
    run_tool -name {GENERATEPROGRAMMINGDATA}
    configure_spiflash -cfg_file "$project_dir/MSS_VIDEO_KIT/JPEG_WRITE/spiflash.cfg"
}
