...
parameter BCLKSCLK_TRN_DIR           = 1;     // 1 - Forward 0 - Backward Direction 
...
// wait state counters
reg [4:0] dly_cnt;
wire [4:0] dly_cnt_val;
assign dly_cnt_val = 5'h1F; 
...
assign vcophsel_dir	 		= BCLKSCLK_TRN_DIR ? 1'b1 : 1'b0;
assign reset_lane 			= ((current_state == SM_PAUS) | (current_state == SM_RESE) | (current_state == SM_MRST))  ? 1'b1 : 1'b0;
assign loadphs_b 			= ((current_state == SM_LOAD) || (current_state == SM_LOAD_DLY)) ? 1'b0 : 1'b1;
...
begin : register_rx
  ...
  begin
    ...
    if (current_state == SM_STOR || current_state == SM_LOAD_DLY || current_state == SM_LOAD || current_state == SM_RREG)
...
begin
  ...
  begin
    if ((current_state == SM_LOAD)  || (current_state == SM_RESE) | (current_state == SM_MRST) | (current_state == SM_VSE2))
      dly_cnt[4:0] <= dly_cnt_val; //5'h1F; //  31 clock cycles after reset
    else if ((current_state == SM_CFLG) | (current_state == SM_PAUS) ) // when entering DEM control stage
      dly_cnt[4:0] <= dly_cnt_val; //5'h4; // 4 clock cycles
...
begin: SM
  ...
  else if (rst_clk_align_trng || (pll_bclksclkalign_train == 1'b0)) begin //START is connected to PLL_LOCK and hence expected to be HIGH 
    current_state   <= SM_IDLE;
    hold_state_flag <= 1'b0; 
    hold_state      <= SM_IDLE;         
  end  
