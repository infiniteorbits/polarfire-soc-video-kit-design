...
//
// FIXES:
//   [1] Assert reading one cycle after entering FEED_PIXELS
//   [2] read_en_i generates read pulses and stops when all pixels have been requested
//   [3] pixel_valid_hold is handled independently of the reading state
//   [4] read_counter and pixel_count are kept separate:
//       read_counter = number of pixels requested from DDR_Read
//       pixel_count  = number of pixels sent to the JLS encoder
...
module jpeg_control_fsm #(
    ...
    // Read interface to DDR_Read
    input  wire         read_ackn_i,
    input  wire         read_done_i,
    output reg          read_en_i,       // Pulse to DDR_Read.read_en_i
    output reg          frame_start_i,   // Pulse to DDR_Read.frame_start_i
    ...
    reg [31:0] pixel_count;    // Number of pixels sent to the JLS encoder
    reg [31:0] read_counter;   // Number of pixels requested from DDR_Read
    ...
    assign total_pixels_comb =
        ({34'd0, i_w} + 1) * ({34'd0, i_h} + 1);

    //============================
    // LINE GAP
    //============================
    // LINE_STORAGE_MODE:
    // 0 = Compact storage, with no padding between lines.
    // 1 = Padded storage, with each line aligned to an 8-byte boundary.
    //
    // The padded mode is suitable for a 64-bit DDR data bus.
    assign line_gap_o = (LINE_STORAGE_MODE == 1'b0)
                      ? horz_resl_o
                      : ((horz_resl_o + 16'd7) & 16'hFFF8);

    //============================
    // FRAME START / READ ENABLE
    //============================
        ...
        if (!rstn) begin
            ...
            if (state == IDLE) begin
                ...
                case (read_state)
                    ...
                    // Wait state before starting a new line
                    RD_IDLE: begin
                        read_en_i        <= 1'b0;
                        read_pixel_timer <= 14'd0;
                        read_gap_timer   <= 8'd0;

                        // Generate the frame-start pulse sequence
                        ...
                        else if (read_line_count < height_lines) begin
                            read_en_i        <= 1'b1;
                            read_pixel_timer <= 14'd0;
                            read_state       <= RD_LINE;
                        end
                    ...
                    // Generate read requests for the current line
                    RD_LINE: begin
                        read_en_i <= 1'b1;
                    ...
                    // Insert the required delay between lines
                    RD_GAP: begin
                        read_en_i <= 1'b0;

                        // Equivalent to the testbench delay:
                        // #(PIXEL_CLK * (280 / 4))
                        // Adjust according to the actual interface timing.
    ...
    // RAM DATA PIPELINE
    //============================
    // Delay the RAM data and valid signal by one clock cycle.
